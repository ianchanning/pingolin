...
self.onmessage = async (e) => {
    ...
    try {
        ...
        if (msg.type === 'INIT') {
            if (!dbPromise)
                dbPromise = initDb(msg.payload?.dbName);
            await dbPromise;
        ...
        if (!db)
            throw new Error('CRITICAL: Database failed to mount.'); // Type assertion guard
        ...
        switch (msg.type) {
            ...
            case 'QUERY_SEARCH': {
                ...
                const effectiveQuery = aliasRows.length > 0 ? aliasRows[0].mapped_tag : payloadStr;
            ...
            case 'LOCAL_UPSERT': {
                console.log('[Worker] LOCAL_UPSERT:', msg.payload.href, msg.payload.description);
                db.transaction((trx) => {
                    const now = Date.now();
            ...
            case 'DEBUG_CLEAR_DB':
                db.transaction((trx) => {
                    trx.exec('DROP TABLE IF EXISTS bookmarks; DROP TABLE IF EXISTS bookmarks_fts; DROP TABLE IF EXISTS tag_aliases; DROP TABLE IF EXISTS metadata;');
                    trx.exec(SCHEMA);
                });
                dispatch({ type: 'EXEC_SUCCESS', id });
                break;
            default:
                console.warn('[Worker] Unknown or removed message type:', msg.type);
        }
    }
    catch (error) {

...
export default {
  async fetch(request: Request, env: any, ctx: any): Promise<Response> {
    ...
    if (!targetPath.startsWith('/v1/')) {
      targetPath =
        '/v1' + (targetPath.startsWith('/') ? targetPath : '/' + targetPath);
    }
    ...
    headers.set(
      'Accept',
      isDatesRequest ? 'application/xml' : 'application/json'
    );
    ...
    try {
      ...
      console.log(
        `[Proxy] Upstream Response: ${response.status} ${response.statusText}`
      );
